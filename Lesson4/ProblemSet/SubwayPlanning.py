...
def subway(**lines):
    ...
    result = {}
    listed_boston = {key: lines[key].split(" ") for key in lines.keys()}

    # For longest ride!
    for line in listed_boston.values():
        finishes += [line[0], line[-1]]

    for color, line in listed_boston.items():
        for i, station in enumerate(line):
            if result.get(station) == None:
                result[station] = {}
            if i == 0:
                if result[station].get(color) == None:
                    result[station][color] = [line[i+1]]
                else:
                    result[station][color] += [line[i+1]]
            elif i == len(line)-1:
                if result[station].get(color) == None:
                ...
                if result[station].get(color) == None:
                    result[station][color] = [line[i-1], line[i+1]]
                else:
                    result[station][color] += [line[i-1], line[i+1]]
    return result
...
def successors(state):
    return {new_state : color for color, states in boston[state].items() for new_state in states}
...
def longest_ride(system):
    ...
    result_states = []
    max_states = 0
    for here, there in itertools.combinations(finishes, r=2):
        current = ride(here, there, system=system)
        if len(path_states(current)) > max_states:
            result_states = current
            max_states = len(path_states(current))
